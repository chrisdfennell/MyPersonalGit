...
- **Chat e Assistente IA** — Pannello chat IA integrato con conversazioni contestuali. Clic destro sul codice per azioni IA: Spiega Codice, Refactoring Codice, Genera Test, Correggi Codice, Aggiungi Documentazione e Spiegazione Inline (Ctrl+Shift+E mostra il widget di spiegazione al cursore). Risposte in streaming con azioni sui blocchi di codice: applica/copia/nuovo file
- **Generazione Test con IA** — Seleziona il codice e genera test unitari completi con rilevamento automatico del framework (xUnit, Jest, pytest, Go testing, Rust #[cfg(test)], JUnit 5, RSpec, PHPUnit)
- **Refactoring Assistito dall'IA** — Seleziona il codice e ottieni suggerimenti di refactoring alimentati dall'IA con applicazione in un clic per sostituire la selezione
- **Debugger Integrato** — Debug multi-linguaggio tramite DAP (Debug Adapter Protocol) con breakpoint (clic sul margine), step over/into/out, continua, ispezione variabili, navigazione dello stack delle chiamate e output della console di debug. Supporta Python (debugpy), C# (netcoredbg), JavaScript/TypeScript (js-debug), Go (Delve), Rust (lldb-dap) e Java (JDWP). Rileva automaticamente il linguaggio dall'estensione del file
...
- **Indicatori Diff nel Margine** — Indicatori verdi, blu e rossi nel margine dell'editor che mostrano righe aggiunte, modificate ed eliminate rispetto all'ultimo commit. Clicca su una barra del margine per vedere il contenuto originale inline
- **Code Snippets** — Frammenti di codice definiti dall'utente memorizzati per browser. Definisci prefisso, linguaggio e corpo in Settings; digita il prefisso e Tab per espandere. Supporta la sintassi dei snippet di VS Code ($1, $2, $0 placeholder)
- **Gestione File** — Albero dei file gerarchico con icone colorate specifiche per linguaggio (oltre 40 tipi di file), annidamento dei file (raggruppa `.razor` + `.razor.css` + `.razor.cs`), ricerca/filtro, caricamento file tramite trascinamento e menu contestuali per nuovo file/cartella/rinomina/elimina
- **Gestione Schede** — Interfaccia multi-scheda con icone file colorate, riordinamento tramite trascinamento, schede fissate, conferma di chiusura per modifiche non salvate, annulla chiusura scheda (Ctrl+Shift+T), menu contestuale clic destro (Chiudi, Chiudi Altre, Chiudi a Destra, Chiudi Salvate) e frecce di scorrimento per l'overflow
...
- **Navigazione del Codice** — Apertura Rapida (Ctrl+P), Palette dei Comandi (Ctrl+Shift+P), Vai alla Riga (Ctrl+G), Go to Definition (F12), Peek Definition (Alt+F12), Find References (Shift+F12), Rename Symbol (F2), pannello struttura/simboli e navigazione breadcrumb
- **Scorciatoie da Tastiera** — Scorciatoie stile VS Code (Ctrl+Shift+P, Ctrl+P, Ctrl+Shift+F, Ctrl+Shift+M, Ctrl+Shift+T, Ctrl+`, F5, F9, Escape) intercettate a livello di documento per impedire i comportamenti predefiniti del browser
...
- **Collaborazione in Tempo Reale** — Più utenti possono modificare lo stesso file contemporaneamente con tracciamento del cursore in tempo reale, evidenziazione della selezione e indicatori di presenza. Basato su SignalR con trasformazione operazionale per modifiche concorrenti senza conflitti. Cursori codificati per colore con etichette del nome utente e riconnessione automatica

...
- **インライン Diff ガター** — エディタガターに緑、青、赤のインジケーターで、最後のコミットと比較した追加行、変更行、削除行を表示
- **ファイル管理** — 階層型ファイルツリー（`.razor` + `.razor.css` + `.razor.cs` のグループ化）、検索/フィルタ、ドラッグ＆ドロップによるファイルアップロード、新規ファイル/フォルダ/リネーム/削除のコンテキストメニュー
- **タブ管理** — マルチタブインターフェース。ドラッグで並べ替え、ピン留めタブ、右クリックコンテキストメニュー（閉じる、他を閉じる、右側を閉じる、保存済みを閉じる）、オーバーフロー時のスクロール矢印
...
- **コードナビゲーション** — クイックオープン（Ctrl+P）、コマンドパレット（Ctrl+Shift+P）、行へ移動（Ctrl+G）、アウトライン/シンボルパネル、パンくずナビゲーション
- **キーボードショートカット** — VS Code スタイルのショートカット（Ctrl+Shift+P、Ctrl+P、Ctrl+Shift+F、Ctrl+Shift+M、Ctrl+`、Escape）をドキュメントレベルでインターセプトし、ブラウザのデフォルト動作を防止

...
- **Star from Explore** — 各リポジトリを開かずに探索ページから直接リポジトリにスターを付けたり外したり
- **Autolink References** — `#123` を自動的に Issue リンクに変換し、リポジトリごとに設定可能なカスタムパターン（例: `JIRA-456` → 外部 URL）にも対応
- **検索** — リポジトリ、Issue、PR、コード全体にわたる全文検索
- **License Detection** — LICENSE ファイルを自動検出し、一般的なライセンス（MIT、Apache-2.0、GPL、BSD、ISC、MPL、Unlicense）をリポジトリサイドバーのバッジで識別
...
- **@Mention Notifications** — コメントでユーザーを @メンションして直接通知を送信
- **マージコンフリクトの解決** — ブラウザ上で直接マージコンフリクトを解決。base/ours/theirs ビュー、クイック承認ボタン、コンフリクトマーカーの検証を備えたビジュアルエディタ
- **Squash Commit Message** — プルリクエストのスカッシュマージ時にコミットメッセージをカスタマイズ
- **Branch Delete After Merge** — プルリクエストのマージ後にソースブランチを自動的に削除するオプション、デフォルトで有効
...
- **CI Status on PR List** — Pull Request リストに各 PR タイトルの横に緑/赤/黄の CI ステータスアイコンを表示
...
- **Saved Replies** — 定型返信を保存し、Issue やプルリクエストへのコメント時にすばやく挿入
- **Batch Issue Operations** — 複数の Issue を選択し、Issue 一覧から一括でクローズまたは再オープン
...
- **PR Template** — `.github/PULL_REQUEST_TEMPLATE.md` からプルリクエストの説明を自動的に事前入力
- **Release Editing** — 作成後にリリースのタイトル、説明、ドラフト/プレリリースフラグを編集
...
- **Activity Pulse** — リポジトリごとの週間サマリーページ。過去7日間のマージされたPR、オープン/クローズされたIssue、コミット、トップコントリビューター、アクティブなブランチを表示

### Web IDE
- **フル機能コードエディタ** — Monaco Editor ベースの IDE。30以上の言語のシンタックスハイライト、複数テーマ、HTML/CSS 向け Emmet 省略展開、ペースト時フォーマット対応
- **Language Server Protocol (LSP)** — IntelliSense、オートコンプリート、ホバー情報、定義へ移動、参照の検索、シグネチャヘルプ、コードフォーマット、シンボルのリネーム、コードアクション、12言語のリアルタイム診断: C# (OmniSharp)、TypeScript/JavaScript、Python、Go、Rust、HTML、CSS、JSON、YAML、Bash、Dockerfile、Markdown
- **AI コード補完** — 入力中にインラインゴーストテキストで候補を表示。OpenAI 互換 API（OpenAI、Ollama、LM Studio など）で動作。管理設定でエンドポイント、API キー、モデルを設定可能。Tab で受け入れ
- **AI チャット & アシスタント** — コンテキスト対応の会話が可能な統合 AI チャットパネル。コードを右クリックして AI アクション: コードの説明、コードのリファクタリング、テストの生成、コードの修正、ドキュメントの追加、インライン説明（Ctrl+Shift+E でカーソル位置に説明ウィジェットを表示）。コードブロックの適用/コピー/新規ファイルアクション付きストリーミング応答
- **AI テスト生成** — コードを選択してフレームワーク自動検出（xUnit、Jest、pytest、Go testing、Rust #[cfg(test)]、JUnit 5、RSpec、PHPUnit）による包括的なユニットテストを生成
- **AI 支援リファクタリング** — コードを選択して AI によるリファクタリング提案をワンクリックで適用し、選択範囲を置き換え
- **統合デバッガ** — DAP（Debug Adapter Protocol）によるマルチ言語デバッグ。ブレークポイント（ガタークリック）、ステップオーバー/イン/アウト、続行、変数検査、コールスタックナビゲーション、デバッグコンソール出力。Python（debugpy）、C#（netcoredbg）、JavaScript/TypeScript（js-debug）、Go（Delve）、Rust（lldb-dap）、Java（JDWP）をサポート。ファイル拡張子から言語を自動検出
- **問題パネル** — ボトムパネルタブで、開いているファイルのすべての LSP 診断（エラー/警告）をファイル別にグループ化して集約。クリックで該当箇所へ移動。ステータスバーにエラー/警告カウントを表示
- **タスクランナー** — ボトムパネルタブでビルド/テストコマンド（dotnet build、npm test、go build、make、pytest など）をリアルタイムストリーミング出力、クリック可能なエラー位置、キャンセルサポート付きで実行
- **ブランチ Diff** — サイドバーパネルで2つのブランチをサイドバイサイドで比較。追加/変更/削除/リネームのステータス付き変更ファイルリストを表示、クリックで Monaco の Diff エディタで差分を表示
- **インライン Diff ガター** — エディタガターに緑、青、赤のインジケーターで、最後のコミットと比較した追加行、変更行、削除行を表示。ガターバーをクリックすると元の内容をインラインで確認可能
- **Code Snippets** — ブラウザごとに保存されるユーザー定義のコードスニペット。Settings でプレフィックス、言語、本文を定義し、プレフィックスを入力して Tab で展開。VS Code スニペット構文（$1、$2、$0 プレースホルダー）をサポート
- **ファイル管理** — 言語別のカラーアイコン（40以上のファイルタイプ）付き階層型ファイルツリー（`.razor` + `.razor.css` + `.razor.cs` のグループ化）、検索/フィルタ、ドラッグ＆ドロップによるファイルアップロード、新規ファイル/フォルダ/リネーム/削除のコンテキストメニュー
- **タブ管理** — カラーファイルアイコン付きマルチタブインターフェース。ドラッグで並べ替え、ピン留めタブ、未保存の変更がある場合の閉じる確認、タブを閉じるの取り消し（Ctrl+Shift+T）、右クリックコンテキストメニュー（閉じる、他を閉じる、右側を閉じる、保存済みを閉じる）、オーバーフロー時のスクロール矢印
- **分割エディタ & Diff ビュー** — 独立スクロールによるサイドバイサイド編集、コミット前の変更比較のための Diff ビュー
- **統合ターミナル** — xterm.js ベースのターミナル。複数のターミナルタブ、WebSocket シェルアクセス、テーマ対応のライト/ダークモード
- **Git 統合** — ブランチ作成、Blame ビュー、ファイル履歴、コミットパネル、ファイル選択付きソース管理、色分けされたレーンラインとブランチラベル付きのビジュアルコミットグラフ
- **マージコンフリクト解決** — インラインの「現在を受け入れ」/「入力を受け入れ」/「両方を受け入れ」ボタン。色分けされたコンフリクト領域（緑/青）
- **検索 & 置換** — 全ファイルにわたるグローバル検索。ファイル拡張子フィルタリング、行単位の結果、すべて置換
- **コードナビゲーション** — クイックオープン（Ctrl+P）、コマンドパレット（Ctrl+Shift+P）、行へ移動（Ctrl+G）、Go to Definition (F12)、Peek Definition (Alt+F12)、Find References (Shift+F12)、Rename Symbol (F2)、アウトライン/シンボルパネル、パンくずナビゲーション
- **キーボードショートカット** — VS Code スタイルのショートカット（Ctrl+Shift+P、Ctrl+P、Ctrl+Shift+F、Ctrl+Shift+M、Ctrl+Shift+T、Ctrl+`、F5、F9、Escape）をドキュメントレベルでインターセプトし、ブラウザのデフォルト動作を防止
- **CSS カラープレビュー** — CSS、SCSS、Less ファイルの hex/rgb/hsl 値の横にインラインカラースウォッチを表示
- **ミニマップハイライト** — 変更行、追加行、コンフリクトマーカーがミニマップガターに色付きマーカーとして表示
- **Markdown & 画像プレビュー** — Markdown ファイルの編集とレンダリングプレビューの切り替え、一般的な画像フォーマットのインライン表示
- **自動保存** — 設定可能な遅延（500ms〜5s）によるオプションの自動コミット。設定またはコマンドパレットで切り替え可能
- **永続的ワークスペース** — 開いているタブ、ピン留めタブ、アクティブファイル、サイドバー状態、パネルモードをブラウザセッション間で記憶
- **リサイズ可能なパネル** — ビジュアルハンドルによるドラッグでサイドバーとボトムパネルのサイズ変更
- **カスタマイズ可能な設定** — フォントファミリー選択（8フォント）、フォントサイズ、タブサイズ、折り返し、ミニマップ、行番号、ブラケットガイド、スティッキースクロール、コードフォールディング、フォントリガチャ、ペースト時フォーマット、空白表示オプション
- **リアルタイムコラボレーション** — 複数ユーザーが同じファイルを同時に編集可能。ライブカーソルトラッキング、選択範囲のハイライト、プレゼンスインジケーター付き。SignalR ベースの操作変換により競合なしの同時編集を実現。ユーザー名ラベル付きの色分けカーソルと自動再接続
- **Zen モード** — 全画面での集中編集。終了ボタンを表示
...
### 管理
- **管理ダッシュボード** — システム設定（データベースプロバイダー、TLS/HTTPS、SSH サーバー、LDAP/AD、フッターページを含む）、ユーザー管理、監査ログ、統計をセクションごとの個別カードで整理
- **組み込み TLS/HTTPS** — 管理設定から直接 HTTPS を有効化。3つの証明書オプション：自己署名（自動生成、2年間有効）、PFX/PKCS#12 ファイル、または PEM 証明書+鍵ペア（例：Let's Encrypt）。Docker ポートマッピング用の内部/外部ポート設定可能、HTTP から HTTPS へのリダイレクトオプション
...
- **Gravatar Avatars** — UI 全体のユーザーアバターがユーザー名に基づく Gravatar アイデンティコンを使用、自動フォールバック付き
...
- **多言語 / i18n** — Web IDEを含む全30以上のページにわたる1,088のリソースキーによる完全なローカライゼーション。11言語を同梱: 英語、スペイン語、フランス語、ドイツ語、日本語、韓国語、中国語（簡体字）、ポルトガル語、ロシア語、イタリア語、トルコ語。`SharedResource.{locale}.resx` ファイルを作成して言語を追加可能。ヘッダーおよびIDEトップバーの言語ピッカーで切り替え
- **Swagger / OpenAPI** — `/swagger` でインタラクティブな API ドキュメントを提供。すべての REST エンドポイントを検索・テスト可能
- **Open Graph Meta Tags** — リポジトリ、Issue、PR ページに og:title と og:description を含め、Slack、Discord、ソーシャルメディアでのリッチリンクプレビューに対応
- **絵文字ショートコード** — GitHub スタイルの絵文字ショートコード（`:white_check_mark:`、`:rocket:` など）がすべての Markdown ビューで実際の絵文字として表示
...
- **Keyboard Shortcuts** — `?` を押すとショートカットヘルプモーダルを表示。`/` で検索にフォーカス、`g i` で Issue へ、`g p` で Pull Request へ、`g h` でホームへ、`g n` で通知へ移動
- **Health Check Endpoint** — `/health` がデータベース接続状態の JSON を返し、Docker/Kubernetes のモニタリングに対応
- **Sitemap.xml** — `/sitemap.xml` で動的な XML サイトマップを生成し、すべての公開リポジトリを検索エンジンのインデックス用にリスト
- **Line Linking** — ファイルビューアで行番号をクリックして共有可能な `#L42` URL を生成。読み込み時に行がハイライトされます
- **File Download** — 適切な Content-Disposition ヘッダー付きでファイルビューアから個別ファイルをダウンロード

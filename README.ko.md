...
- **인라인 Diff 거터** — 편집기 거터에 녹색, 파란색, 빨간색 표시기로 마지막 커밋 대비 추가, 수정, 삭제된 줄을 표시
- **파일 관리** — 파일 중첩이 포함된 계층적 파일 트리(`.razor` + `.razor.css` + `.razor.cs` 그룹화), 검색/필터, 드래그 앤 드롭 파일 업로드, 새 파일/폴더/이름 변경/삭제를 위한 컨텍스트 메뉴
- **탭 관리** — 드래그로 재정렬, 고정 탭, 우클릭 컨텍스트 메뉴(닫기, 다른 탭 닫기, 오른쪽 탭 닫기, 저장된 탭 닫기), 오버플로 시 스크롤 화살표를 갖춘 다중 탭 인터페이스
...
- **코드 탐색** — 빠른 열기(Ctrl+P), 명령 팔레트(Ctrl+Shift+P), 줄로 이동(Ctrl+G), 개요/심볼 패널 및 브레드크럼 탐색
- **키보드 단축키** — VS Code 스타일 단축키(Ctrl+Shift+P, Ctrl+P, Ctrl+Shift+F, Ctrl+Shift+M, Ctrl+`, Escape)를 문서 수준에서 가로채 브라우저 기본 동작 방지

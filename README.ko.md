...
- **Star from Explore** — 각 리포지토리를 열지 않고 탐색 페이지에서 직접 리포지토리에 스타를 추가하거나 제거
...
- **License Detection** — LICENSE 파일을 자동 감지하고 일반적인 라이선스(MIT, Apache-2.0, GPL, BSD, ISC, MPL, Unlicense)를 리포지토리 사이드바의 배지로 식별
...
- **@Mention Notifications** — 댓글에서 사용자를 @멘션하여 직접 알림 전송
- **머지 충돌 해결** — 브라우저에서 직접 머지 충돌 해결. base/ours/theirs 뷰, 빠른 승인 버튼, 충돌 마커 검증을 갖춘 비주얼 에디터
- **Squash Commit Message** — Pull Request를 스쿼시 머지할 때 커밋 메시지를 사용자 정의
- **Branch Delete After Merge** — Pull Request 병합 후 소스 브랜치를 자동으로 삭제하는 옵션, 기본적으로 활성화
...
- **CI Status on PR List** — Pull Request 목록에서 각 PR 제목 옆에 녹색/빨간색/노란색 CI 상태 아이콘 표시
...
- **Saved Replies** — 미리 작성한 답변을 저장하고 Issue 또는 Pull Request 댓글 작성 시 빠르게 삽입
- **Batch Issue Operations** — 여러 Issue를 선택하고 Issue 목록에서 일괄 닫기 또는 다시 열기
...
- **PR Template** — `.github/PULL_REQUEST_TEMPLATE.md`에서 Pull Request 설명을 자동으로 미리 채우기
- **Release Editing** — 생성 후 릴리스 제목, 설명, 드래프트/프리릴리스 플래그 편집
...
- **Activity Pulse** — 리포지토리별 주간 요약 페이지로 지난 7일간의 머지된 PR, 열린/닫힌 Issue, 커밋, 상위 기여자, 활성 브랜치를 표시

### Web IDE
- **완전한 기능의 코드 편집기** — Monaco Editor 기반 IDE로 30개 이상의 언어 구문 강조, 다양한 테마, HTML/CSS용 Emmet 약어 확장 및 붙여넣기 시 서식 지정
- **Language Server Protocol (LSP)** — IntelliSense, 자동 완성, 호버 정보, 정의로 이동, 참조 찾기, 서명 도움말, 코드 서식, 심볼 이름 변경, 코드 작업 및 12개 언어의 실시간 진단: C# (OmniSharp), TypeScript/JavaScript, Python, Go, Rust, HTML, CSS, JSON, YAML, Bash, Dockerfile, Markdown
- **AI 코드 완성** — 입력 중 인라인 고스트 텍스트 제안, OpenAI 호환 API(OpenAI, Ollama, LM Studio 등)로 구동. 관리자 설정에서 엔드포인트, API 키, 모델 구성 가능. Tab으로 수락
- **AI 채팅 & 어시스턴트** — 컨텍스트 인식 대화가 가능한 통합 AI 채팅 패널. 코드를 우클릭하여 AI 작업 수행: 코드 설명, 코드 리팩터링, 테스트 생성, 코드 수정, 문서 추가, 인라인 설명(Ctrl+Shift+E로 커서 위치에 설명 위젯 표시). 코드 블록 적용/복사/새 파일 작업이 포함된 스트리밍 응답
- **AI 테스트 생성** — 코드를 선택하고 프레임워크 자동 감지(xUnit, Jest, pytest, Go testing, Rust #[cfg(test)], JUnit 5, RSpec, PHPUnit)를 통해 포괄적인 단위 테스트 생성
- **AI 지원 리팩터링** — 코드를 선택하고 AI 기반 리팩터링 제안을 받아 원클릭으로 선택 영역을 대체 적용
- **통합 디버거** — DAP(Debug Adapter Protocol)를 통한 다국어 디버깅으로 브레이크포인트(거터 클릭), 스텝 오버/인투/아웃, 계속, 변수 검사, 콜 스택 탐색 및 디버그 콘솔 출력 지원. Python(debugpy), C#(netcoredbg), JavaScript/TypeScript(js-debug), Go(Delve), Rust(lldb-dap), Java(JDWP) 지원. 파일 확장자로 언어 자동 감지
- **문제 패널** — 열린 파일의 모든 LSP 진단(오류/경고)을 파일별로 그룹화하여 집계하는 하단 패널 탭. 클릭하여 해당 위치로 이동. 상태 표시줄에 오류/경고 카운트 표시
- **작업 실행기** — 빌드/테스트 명령(dotnet build, npm test, go build, make, pytest 등)을 실시간 스트리밍 출력, 클릭 가능한 오류 위치, 취소 지원과 함께 실행하는 하단 패널 탭
- **브랜치 Diff** — 두 브랜치를 나란히 비교하는 사이드바 패널. 추가/수정/삭제/이름 변경 상태가 포함된 변경 파일 목록을 표시하고, 클릭하면 Monaco의 Diff 편집기에서 차이점 확인
- **인라인 Diff 거터** — 편집기 거터에 녹색, 파란색, 빨간색 표시기로 마지막 커밋 대비 추가, 수정, 삭제된 줄을 표시. 거터 바를 클릭하면 원본 내용을 인라인으로 확인 가능
- **Code Snippets** — 브라우저별로 저장되는 사용자 정의 코드 스니펫. Settings에서 접두사, 언어, 본문을 정의하고, 접두사를 입력한 후 Tab으로 확장. VS Code 스니펫 구문($1, $2, $0 플레이스홀더) 지원
- **파일 관리** — 언어별 컬러 아이콘(40개 이상의 파일 유형), 파일 중첩이 포함된 계층적 파일 트리(`.razor` + `.razor.css` + `.razor.cs` 그룹화), 검색/필터, 드래그 앤 드롭 파일 업로드, 새 파일/폴더/이름 변경/삭제를 위한 컨텍스트 메뉴
- **탭 관리** — 컬러 파일 아이콘, 드래그로 재정렬, 고정 탭, 저장하지 않은 변경 사항 닫기 확인, 탭 닫기 취소(Ctrl+Shift+T), 우클릭 컨텍스트 메뉴(닫기, 다른 탭 닫기, 오른쪽 탭 닫기, 저장된 탭 닫기), 오버플로 시 스크롤 화살표를 갖춘 다중 탭 인터페이스
- **분할 편집기 & Diff 보기** — 독립적인 스크롤로 나란히 편집, 커밋 전 변경 사항을 비교하는 Diff 보기
- **통합 터미널** — xterm.js 기반 터미널로 다중 터미널 탭, WebSocket 셸 접근 및 테마 인식 라이트/다크 모드 지원
- **Git 통합** — 브랜치 생성, blame 보기, 파일 히스토리, 커밋 패널, 파일 선택이 가능한 소스 제어, 컬러 레인 라인과 브랜치 라벨이 있는 시각적 커밋 그래프
- **머지 충돌 해결** — 인라인 현재 수락 / 수신 수락 / 양쪽 수락 버튼과 색상 구분된 충돌 영역(녹색/파란색)
- **검색 & 바꾸기** — 파일 확장자 필터링, 줄별 결과, 모두 바꾸기를 지원하는 모든 파일 대상 전역 검색
- **코드 탐색** — 빠른 열기(Ctrl+P), 명령 팔레트(Ctrl+Shift+P), 줄로 이동(Ctrl+G), Go to Definition (F12), Peek Definition (Alt+F12), Find References (Shift+F12), Rename Symbol (F2), 개요/심볼 패널 및 브레드크럼 탐색
- **키보드 단축키** — VS Code 스타일 단축키(Ctrl+Shift+P, Ctrl+P, Ctrl+Shift+F, Ctrl+Shift+M, Ctrl+Shift+T, Ctrl+`, F5, F9, Escape)를 문서 수준에서 가로채 브라우저 기본 동작 방지
- **CSS 색상 미리보기** — CSS, SCSS, Less 파일의 hex/rgb/hsl 값 옆에 인라인 색상 견본 표시
- **미니맵 하이라이트** — 수정된 줄, 추가된 줄, 충돌 마커가 미니맵 거터에 색상 마커로 표시
- **Markdown & 이미지 미리보기** — Markdown 파일의 편집과 렌더링된 미리보기 간 전환; 일반적인 형식의 인라인 이미지 표시
- **자동 저장** — 설정 가능한 지연(500ms~5s)으로 선택적 자동 커밋, 설정 또는 명령 팔레트에서 전환 가능
- **영구 작업 공간** — 열린 탭, 고정 탭, 활성 파일, 사이드바 상태, 패널 모드를 브라우저 세션 간에 기억
- **크기 조절 가능한 패널** — 시각적 핸들로 사이드바와 하단 패널을 드래그하여 크기 조절
- **사용자 정의 설정** — 글꼴 패밀리 선택기(8종), 글꼴 크기, 탭 크기, 줄 바꿈, 미니맵, 줄 번호, 괄호 가이드, 고정 스크롤, 코드 접기, 글꼴 합자, 붙여넣기 시 서식, 공백 표시 옵션
- **실시간 협업** — 여러 사용자가 동시에 같은 파일을 편집할 수 있으며, 실시간 커서 추적, 선택 영역 하이라이트, 접속 상태 표시기 제공. SignalR 기반의 운영 변환으로 충돌 없는 동시 편집 구현. 사용자 이름 라벨이 있는 색상 구분 커서와 자동 재연결
- **Zen 모드** — 방해 없는 전체 화면 편집, 종료 버튼 표시
...
### 관리
- **관리 대시보드** — 시스템 설정(데이터베이스 프로바이더, TLS/HTTPS, SSH 서버, LDAP/AD, 푸터 페이지 포함), 사용자 관리, 감사 로그, 통계를 섹션별 개별 카드로 구성
- **내장 TLS/HTTPS** — 관리 설정에서 직접 HTTPS 활성화. 3가지 인증서 옵션: 자체 서명(자동 생성, 2년 유효), PFX/PKCS#12 파일, 또는 PEM 인증서+키 쌍(예: Let's Encrypt). Docker 포트 매핑을 위한 내부/외부 포트 설정 가능, 선택적 HTTP에서 HTTPS로의 리다이렉트
- **커스터마이즈 가능한 푸터 페이지** — Admin > Settings에서 Markdown 콘텐츠를 편집할 수 있는 이용 약관, 개인정보처리방침, 문서, 연락처 페이지
- **사용자 프로필** — 사용자별 기여 히트맵, 활동 피드, 통계
- **Gravatar Avatars** — UI 전체의 사용자 아바타가 사용자 이름 기반 Gravatar 아이덴티콘을 사용하며, 자동 폴백 지원
...
- **다국어 / i18n** — Web IDE를 포함한 전체 30개 이상의 페이지에 걸친 1,088개 리소스 키의 완전한 현지화. 11개 언어 기본 제공: 영어, 스페인어, 프랑스어, 독일어, 일본어, 한국어, 중국어(간체), 포르투갈어, 러시아어, 이탈리아어, 터키어. `SharedResource.{locale}.resx` 파일을 생성하여 언어 추가 가능. 헤더 및 IDE 상단 바의 언어 선택기로 전환
- **Swagger / OpenAPI** — `/swagger`에서 인터랙티브 API 문서 제공. 모든 REST 엔드포인트를 검색하고 테스트 가능
- **Open Graph Meta Tags** — 리포지토리, Issue, PR 페이지에 og:title 및 og:description을 포함하여 Slack, Discord, 소셜 미디어에서 풍부한 링크 미리보기 제공
- **이모지 단축코드** — GitHub 스타일 이모지 단축코드(`:white_check_mark:`, `:rocket:` 등)가 모든 Markdown 뷰에서 실제 이모지로 렌더링
...
- **Keyboard Shortcuts** — `?`를 눌러 단축키 도움말 모달을 표시. `/`로 검색에 포커스, `g i`로 Issue, `g p`로 Pull Request, `g h`로 홈, `g n`으로 알림으로 이동
- **Health Check Endpoint** — `/health`가 Docker/Kubernetes 모니터링을 위한 데이터베이스 연결 상태의 JSON을 반환
- **Sitemap.xml** — `/sitemap.xml`에서 모든 공개 리포지토리를 나열하는 동적 XML 사이트맵으로 검색 엔진 인덱싱 지원
- **Line Linking** — 파일 뷰어에서 줄 번호를 클릭하여 로드 시 줄 하이라이팅이 적용되는 공유 가능한 `#L42` URL 생성
- **File Download** — 적절한 Content-Disposition 헤더와 함께 파일 뷰어에서 개별 파일 다운로드

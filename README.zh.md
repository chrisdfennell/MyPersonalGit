...
- **Language Server Protocol (LSP)** — IntelliSense、自动补全、悬停信息、转到定义、查找引用、签名帮助、代码格式化、重命名符号、代码操作以及 12 种语言的实时诊断：C# (OmniSharp)、TypeScript/JavaScript、Python、Go、Rust、HTML、CSS、JSON、YAML、Bash、Dockerfile 和 Markdown
- **AI 代码补全** — 输入时显示内联幽灵文本建议，由任何 OpenAI 兼容 API（OpenAI、Ollama、LM Studio 等）提供支持。可在管理设置中配置端点、API 密钥和模型。按 Tab 接受
- **AI 聊天与助手** — 集成的 AI 聊天面板，支持上下文感知对话。右键点击代码可执行 AI 操作：解释代码、重构代码、生成测试、修复代码、添加文档和内联解释（Ctrl+Shift+E 在光标处显示解释小部件）。支持流式响应，代码块可执行应用/复制/新建文件操作
- **AI 测试生成** — 选择代码并生成全面的单元测试，自动检测测试框架（xUnit、Jest、pytest、Go testing、Rust #[cfg(test)]、JUnit 5、RSpec、PHPUnit）
- **AI 辅助重构** — 选择代码并获取 AI 驱动的重构建议，一键应用即可替换所选内容
- **集成调试器** — 通过 DAP（Debug Adapter Protocol）进行多语言调试，支持断点（点击边栏）、单步跳过/进入/跳出、继续、变量检查、调用栈导航和调试控制台输出。支持 Python（debugpy）、C#（netcoredbg）、JavaScript/TypeScript（js-debug）、Go（Delve）、Rust（lldb-dap）和 Java（JDWP）。根据文件扩展名自动检测语言
- **问题面板** — 底部面板标签页，汇总所有打开文件的 LSP 诊断（错误/警告），按文件分组并支持点击导航。状态栏显示错误/警告计数
- **任务运行器** — 底部面板标签页，用于运行构建/测试命令（dotnet build、npm test、go build、make、pytest 等），支持实时流式输出、可点击的错误位置和取消操作
- **分支 Diff** — 侧边栏面板，用于并排比较两个分支。显示带有添加/修改/删除/重命名状态的更改文件列表，点击可在 Monaco 的 Diff 编辑器中查看差异
- **内联 Diff 边栏标记** — 编辑器边栏中的绿色、蓝色和红色指示器，显示与上次提交相比的新增行、修改行和删除行。点击边栏条可内联查看原始内容
- **Code Snippets** — 按浏览器存储的用户自定义代码片段。在 Settings 中定义前缀、语言和正文；输入前缀后按 Tab 展开。支持 VS Code 代码片段语法（$1、$2、$0 占位符）
- **文件管理** — 层级文件树，支持语言特定的彩色图标（40 多种文件类型）、文件嵌套（将 `.razor` + `.razor.css` + `.razor.cs` 归组）、搜索/过滤、拖放上传文件，以及新建文件/文件夹/重命名/删除的右键菜单
- **标签页管理** — 多标签页界面，支持彩色文件图标、拖拽排序、固定标签页、关闭未保存更改时的确认提示、撤销关闭标签页（Ctrl+Shift+T）、右键菜单（关闭、关闭其他、关闭右侧、关闭已保存的），以及溢出时的滚动箭头
- **分屏编辑器 & Diff 视图** — 独立滚动的并排编辑，以及提交前比较更改的 Diff 视图
- **集成终端** — 基于 xterm.js 的终端，支持多终端标签页、WebSocket Shell 访问以及主题感知的亮色/暗色模式
...
- **代码导航** — 快速打开（Ctrl+P）、命令面板（Ctrl+Shift+P）、跳转到行（Ctrl+G）、Go to Definition (F12)、Peek Definition (Alt+F12)、Find References (Shift+F12)、Rename Symbol (F2)、大纲/符号面板和面包屑导航
- **键盘快捷键** — VS Code 风格的快捷键（Ctrl+Shift+P、Ctrl+P、Ctrl+Shift+F、Ctrl+Shift+M、Ctrl+Shift+T、Ctrl+`、F5、F9、Escape）在文档级别拦截以防止浏览器默认行为
...
- **实时协作** — 多个用户可以同时编辑同一文件，支持实时光标跟踪、选区高亮和在线状态指示器。基于 SignalR 的操作转换实现无冲突并发编辑。带有用户名标签的颜色编码光标和自动重连
- **Zen 模式** — 无干扰全屏编辑，带有可见的退出按钮

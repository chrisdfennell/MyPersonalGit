...
- **Star from Explore** — Ajouter et retirer des étoiles aux dépôts directement depuis la page Explorer sans ouvrir chaque dépôt
...
- **@Mention Notifications** — @mentionnez des utilisateurs dans les commentaires pour leur envoyer une notification directe
...
- **Branch Delete After Merge** — Option pour supprimer automatiquement la branche source après la fusion d'une pull request, activée par défaut
...
- **CI Status on PR List** — La liste des pull requests affiche des icônes de statut CI vertes/rouges/jaunes à côté de chaque titre de PR
...
- **PR Template** — Pré-remplir automatiquement les descriptions de pull requests depuis `.github/PULL_REQUEST_TEMPLATE.md`
...
- **Chat et Assistant IA** — Panneau de chat IA intégré avec des conversations contextuelles. Clic droit sur le code pour les actions IA : Expliquer le Code, Refactoriser le Code, Générer des Tests, Corriger le Code, Ajouter la Documentation et Explication en Ligne (Ctrl+Shift+E affiche un widget d'explication au curseur). Réponses en streaming avec actions sur les blocs de code : appliquer/copier/nouveau fichier
- **Génération de Tests par IA** — Sélectionnez du code et générez des tests unitaires complets avec détection automatique du framework (xUnit, Jest, pytest, Go testing, Rust #[cfg(test)], JUnit 5, RSpec, PHPUnit)
- **Refactorisation Assistée par IA** — Sélectionnez du code et obtenez des suggestions de refactorisation alimentées par l'IA avec application en un clic pour remplacer la sélection
- **Débogueur Intégré** — Débogage multi-langage via DAP (Debug Adapter Protocol) avec points d'arrêt (clic dans la gouttière), pas à pas principal/entrant/sortant, continuer, inspection des variables, navigation dans la pile d'appels et sortie de la console de débogage. Supporte Python (debugpy), C# (netcoredbg), JavaScript/TypeScript (js-debug), Go (Delve), Rust (lldb-dap) et Java (JDWP). Détection automatique du langage par extension de fichier
...
- **Indicateurs Diff dans la Gouttière** — Indicateurs verts, bleus et rouges dans la gouttière de l'éditeur montrant les lignes ajoutées, modifiées et supprimées par rapport au dernier commit. Cliquez sur une barre de gouttière pour voir le contenu original en ligne
- **Code Snippets** — Extraits de code définis par l'utilisateur stockés par navigateur. Définissez préfixe, langage et corps dans Settings ; tapez le préfixe et Tab pour développer. Supporte la syntaxe de snippets VS Code ($1, $2, $0 placeholders)
- **Gestion des Fichiers** — Arborescence hiérarchique avec icônes colorées spécifiques au langage (plus de 40 types de fichiers), imbrication de fichiers (regroupe `.razor` + `.razor.css` + `.razor.cs`), recherche/filtrage, envoi de fichiers par glisser-déposer et menus contextuels pour nouveau fichier/dossier/renommer/supprimer
- **Gestion des Onglets** — Interface multi-onglets avec icônes de fichier colorées, réorganisation par glissement, onglets épinglés, confirmation de fermeture pour les modifications non enregistrées, annuler la fermeture d'onglet (Ctrl+Shift+T), menu contextuel clic droit (Fermer, Fermer les Autres, Fermer à Droite, Fermer les Enregistrés) et chevrons de défilement pour le débordement
...
- **Navigation dans le Code** — Ouverture rapide (Ctrl+P), Palette de commandes (Ctrl+Shift+P), Aller à la ligne (Ctrl+G), Go to Definition (F12), Peek Definition (Alt+F12), Find References (Shift+F12), Rename Symbol (F2), panneau de plan/symboles et navigation par fil d'Ariane
- **Raccourcis Clavier** — Raccourcis style VS Code (Ctrl+Shift+P, Ctrl+P, Ctrl+Shift+F, Ctrl+Shift+M, Ctrl+Shift+T, Ctrl+`, F5, F9, Escape) interceptés au niveau du document pour empêcher les comportements par défaut du navigateur
...
- **Sauvegarde Automatique** — Auto-commit optionnel avec délai configurable (500ms–5s), activable dans les paramètres ou la palette de commandes
...
- **Collaboration en Temps Réel** — Plusieurs utilisateurs peuvent modifier le même fichier simultanément avec suivi des curseurs en direct, mise en évidence des sélections et indicateurs de présence. Alimenté par SignalR avec transformation opérationnelle pour des éditions concurrentes sans conflit. Curseurs codés par couleur avec étiquettes de nom d'utilisateur et reconnexion automatique
...
### Administration
- **Tableau de bord Admin** — Paramètres système (incluant le fournisseur de base de données, TLS/HTTPS, le serveur SSH, LDAP/AD, les pages de pied de page), gestion des utilisateurs, journaux d'audit et statistiques organisés en cartes séparées par section
- **TLS/HTTPS Intégré** — Activer HTTPS directement depuis les paramètres d'administration avec trois options de certificat : auto-signé (généré automatiquement, validité 2 ans), fichier PFX/PKCS#12 ou paire certificat PEM + clé (ex. Let's Encrypt). Ports internes/externes configurables pour le mappage de ports Docker, redirection HTTP vers HTTPS optionnelle
...
- **Gravatar Avatars** — Les avatars utilisateur dans toute l'interface utilisent les identicons Gravatar basés sur le nom d'utilisateur, avec repli automatique
...
- **Multi-langue / i18n** — Localisation complète sur les plus de 30 pages, y compris l'IDE Web, avec 1 088 clés de ressources. Livré avec 11 langues : anglais, espagnol, français, allemand, japonais, coréen, chinois (simplifié), portugais, russe, italien et turc. Sélecteur de langue dans l'en-tête et dans la barre supérieure de l'IDE. Ajoutez-en d'autres en créant des fichiers `SharedResource.{locale}.resx`
...
- **Emoji Shortcodes** — Codes courts emoji style GitHub (`:white_check_mark:`, `:rocket:`, etc.) rendus en vrais emoji dans toutes les vues Markdown

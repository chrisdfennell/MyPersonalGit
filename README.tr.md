...
- **AI Sohbet ve Asistan** — Bağlama duyarlı sohbetler sunan entegre AI sohbet paneli. Kod üzerine sağ tıklayarak AI eylemleri: Kodu Açıkla, Kodu Yeniden Düzenle, Test Oluştur, Kodu Düzelt, Belge Ekle ve Satır İçi Açıklama (Ctrl+Shift+E imlecin bulunduğu yerde açıklama widget'ını gösterir). Kod bloğu uygulama/kopyalama/yeni dosya eylemleriyle akış yanıtları
- **AI Test Oluşturma** — Kod seçin ve çerçeve otomatik algılama (xUnit, Jest, pytest, Go testing, Rust #[cfg(test)], JUnit 5, RSpec, PHPUnit) ile kapsamlı birim testleri oluşturun
- **AI Destekli Yeniden Düzenleme** — Kod seçin ve seçimi değiştirmek için tek tıkla uygulama ile AI destekli yeniden düzenleme önerileri alın
- **Entegre Hata Ayıklayıcı** — DAP (Debug Adapter Protocol) aracılığıyla çoklu dil hata ayıklama; kesme noktaları (kenar boşluğuna tıklama), adım atlama/içine/dışına, devam et, değişken inceleme, çağrı yığını navigasyonu ve hata ayıklama konsol çıktısı. Python (debugpy), C# (netcoredbg), JavaScript/TypeScript (js-debug), Go (Delve), Rust (lldb-dap) ve Java (JDWP) desteği. Dosya uzantısından dili otomatik algılar
...
- **Gerçek Zamanlı İşbirliği** — Birden fazla kullanıcı aynı dosyayı eş zamanlı olarak düzenleyebilir; canlı imleç takibi, seçim vurgulama ve durum göstergeleri ile. Çakışmasız eş zamanlı düzenlemeler için operasyonel dönüşüm kullanan SignalR tabanlı altyapı. Kullanıcı adı etiketleri ve otomatik yeniden bağlanma ile renk kodlu imleçler
...
- **Çoklu Dil / i18n** — Web IDE dahil tüm 30+ sayfada 1.088 kaynak anahtarı ile tam yerelleştirme. 11 dil ile birlikte gelir: İngilizce, İspanyolca, Fransızca, Almanca, Japonca, Korece, Çince (Basitleştirilmiş), Portekizce, Rusça, İtalyanca ve Türkçe. `SharedResource.{locale}.resx` dosyaları oluşturarak daha fazla dil ekleyin. Başlık ve IDE üst çubuğundaki dil seçici ile geçiş yapın

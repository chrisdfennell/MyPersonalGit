...
- **Star from Explore** — Marca y desmarca repositorios con estrella directamente desde la página Explorar sin abrir cada repositorio
...
- **License Detection** — Detecta automaticamente archivos LICENSE e identifica licencias comunes (MIT, Apache-2.0, GPL, BSD, ISC, MPL, Unlicense) con una insignia en la barra lateral del repositorio
...
- **@Mention Notifications** — @menciona usuarios en comentarios para enviarles una notificación directa
...
- **Squash Commit Message** — Personaliza el mensaje de commit al hacer squash-merge de un pull request
- **Branch Delete After Merge** — Opción para eliminar automáticamente la rama de origen después de fusionar un pull request, habilitada por defecto
...
- **CI Status on PR List** — La lista de pull requests muestra iconos de estado CI verdes/rojos/amarillos junto a cada título de PR
...
- **Saved Replies** — Guarda respuestas predefinidas e insértalas rápidamente al comentar en issues o pull requests
- **Batch Issue Operations** — Selecciona múltiples issues y ciérralos o reábrelos en bloque desde la lista de issues
...
- **PR Template** — Rellena automáticamente las descripciones de pull requests desde `.github/PULL_REQUEST_TEMPLATE.md`
- **Release Editing** — Edita titulos, descripciones y banderas de borrador/pre-lanzamiento de releases despues de su creacion
...
- **Chat y Asistente de IA** — Panel de chat de IA integrado con conversaciones contextuales. Haz clic derecho en el codigo para acciones de IA: Explicar Codigo, Refactorizar Codigo, Generar Tests, Corregir Codigo, Agregar Documentacion y Explicacion en Linea (Ctrl+Shift+E muestra un widget de explicacion en el cursor). Respuestas en streaming con acciones de bloque de codigo para aplicar/copiar/nuevo archivo
- **Generacion de Tests con IA** — Selecciona codigo y genera tests unitarios completos con autodeteccion de framework (xUnit, Jest, pytest, Go testing, Rust #[cfg(test)], JUnit 5, RSpec, PHPUnit)
- **Refactorizacion Asistida por IA** — Selecciona codigo y obtén sugerencias de refactorizacion impulsadas por IA con aplicacion en un clic para reemplazar la seleccion
- **Depurador Integrado** — Depuracion multilenguaje via DAP (Debug Adapter Protocol) con puntos de interrupcion (clic en el margen), paso por encima/dentro/fuera, continuar, inspeccion de variables, navegacion de pila de llamadas y salida de consola de depuracion. Soporta Python (debugpy), C# (netcoredbg), JavaScript/TypeScript (js-debug), Go (Delve), Rust (lldb-dap) y Java (JDWP). Detecta automaticamente el lenguaje por la extension del archivo
...
- **Indicadores Diff en el Margen** — Indicadores verdes, azules y rojos en el margen del editor mostrando lineas agregadas, modificadas y eliminadas respecto al ultimo commit. Haz clic en una barra del margen para ver el contenido original en linea
- **Code Snippets** — Fragmentos de codigo definidos por el usuario almacenados por navegador. Define prefijo, lenguaje y cuerpo en Settings; escribe el prefijo y Tab para expandir. Soporta sintaxis de snippets de VS Code ($1, $2, $0 placeholders)
- **Gestion de Archivos** — Arbol de archivos jerarquico con iconos coloreados especificos por lenguaje (mas de 40 tipos de archivo), anidacion de archivos (agrupa `.razor` + `.razor.css` + `.razor.cs`), busqueda/filtro, carga de archivos mediante arrastrar y soltar, y menus contextuales para nuevo archivo/carpeta/renombrar/eliminar
- **Gestion de Pestanas** — Interfaz con multiples pestanas con iconos de archivo coloreados, reordenamiento por arrastre, pestanas fijadas, confirmacion al cerrar con cambios sin guardar, deshacer cierre de pestana (Ctrl+Shift+T), menu contextual de clic derecho (Cerrar, Cerrar Otras, Cerrar a la Derecha, Cerrar Guardadas), y flechas de desplazamiento para desbordamiento
...
- **Navegacion de Codigo** — Apertura Rapida (Ctrl+P), Paleta de Comandos (Ctrl+Shift+P), Ir a Linea (Ctrl+G), Go to Definition (F12), Peek Definition (Alt+F12), Find References (Shift+F12), Rename Symbol (F2), panel de esquema/simbolos y navegacion por migas de pan
- **Atajos de Teclado** — Atajos estilo VS Code (Ctrl+Shift+P, Ctrl+P, Ctrl+Shift+F, Ctrl+Shift+M, Ctrl+Shift+T, Ctrl+`, F5, F9, Escape) interceptados a nivel de documento para evitar comportamientos por defecto del navegador
...
- **Colaboracion en Tiempo Real** — Multiples usuarios pueden editar el mismo archivo simultaneamente con seguimiento de cursor en vivo, resaltado de seleccion e indicadores de presencia. Basado en SignalR con transformacion operacional para ediciones concurrentes sin conflictos. Cursores codificados por color con etiquetas de nombre de usuario y reconexion automatica
...
- **Multi-Idioma / i18n** — Localizacion completa en las mas de 30 paginas, incluyendo el IDE Web, con 1.088 claves de recursos. Incluye 11 idiomas: ingles, espanol, frances, aleman, japones, coreano, chino (simplificado), portugues, ruso, italiano y turco. Selector de idioma en el encabezado y en la barra superior del IDE. Agrega mas creando archivos `SharedResource.{locale}.resx`
...
- **Keyboard Shortcuts** — Presiona `?` para un modal de ayuda de atajos. `/` enfoca la busqueda, `g i` va a Issues, `g p` a Pull Requests, `g h` a Inicio, `g n` a Notificaciones
- **Health Check Endpoint** — `/health` devuelve JSON con el estado de conectividad de la base de datos para monitoreo de Docker/Kubernetes
- **Sitemap.xml** — Sitemap XML dinámico en `/sitemap.xml` que lista todos los repositorios públicos para la indexación de motores de búsqueda
- **Line Linking** — Haz clic en los numeros de linea en el visor de archivos para generar URLs compartibles `#L42` con resaltado de linea al cargar
- **File Download** — Descarga archivos individuales desde el visor de archivos con encabezados Content-Disposition apropiados

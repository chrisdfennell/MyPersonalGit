...
- **Batch Issue Operations** — Selecione múltiplos issues e feche-os ou reabra-os em massa a partir da lista de issues
...
- **Chat e Assistente de IA** — Painel de chat de IA integrado com conversas contextuais. Clique com o botão direito no código para ações de IA: Explicar Código, Refatorar Código, Gerar Testes, Corrigir Código, Adicionar Documentação e Explicação Inline (Ctrl+Shift+E exibe o widget de explicação no cursor). Respostas em streaming com ações de bloco de código para aplicar/copiar/novo arquivo
- **Geração de Testes com IA** — Selecione código e gere testes unitários abrangentes com detecção automática de framework (xUnit, Jest, pytest, Go testing, Rust #[cfg(test)], JUnit 5, RSpec, PHPUnit)
- **Refatoração Assistida por IA** — Selecione código e obtenha sugestões de refatoração alimentadas por IA com aplicação em um clique para substituir a seleção
- **Depurador Integrado** — Depuração multi-linguagem via DAP (Debug Adapter Protocol) com pontos de interrupção (clique na calha), step over/into/out, continuar, inspeção de variáveis, navegação na pilha de chamadas e saída do console de depuração. Suporta Python (debugpy), C# (netcoredbg), JavaScript/TypeScript (js-debug), Go (Delve), Rust (lldb-dap) e Java (JDWP). Detecta automaticamente a linguagem pela extensão do arquivo
...
- **Indicadores Diff na Calha** — Indicadores verdes, azuis e vermelhos na calha do editor mostrando linhas adicionadas, modificadas e excluídas em relação ao último commit. Clique em uma barra da calha para ver o conteúdo original inline
- **Code Snippets** — Trechos de código definidos pelo usuário armazenados por navegador. Defina prefixo, linguagem e corpo em Settings; digite o prefixo e Tab para expandir. Suporta sintaxe de snippets do VS Code ($1, $2, $0 placeholders)
- **Gerenciamento de Arquivos** — Árvore de arquivos hierárquica com ícones coloridos específicos por linguagem (mais de 40 tipos de arquivo), aninhamento de arquivos (agrupa `.razor` + `.razor.css` + `.razor.cs`), busca/filtro, upload de arquivos por arrastar e soltar e menus de contexto para novo arquivo/pasta/renomear/excluir
- **Gerenciamento de Abas** — Interface com múltiplas abas com ícones de arquivo coloridos, reordenação por arraste, abas fixadas, confirmação ao fechar com alterações não salvas, desfazer fechamento de aba (Ctrl+Shift+T), menu de contexto com clique direito (Fechar, Fechar Outras, Fechar à Direita, Fechar Salvas) e setas de rolagem para overflow
...
- **Navegação de Código** — Abertura Rápida (Ctrl+P), Paleta de Comandos (Ctrl+Shift+P), Ir para Linha (Ctrl+G), Go to Definition (F12), Peek Definition (Alt+F12), Find References (Shift+F12), Rename Symbol (F2), painel de estrutura/símbolos e navegação por trilhas
- **Atalhos de Teclado** — Atalhos estilo VS Code (Ctrl+Shift+P, Ctrl+P, Ctrl+Shift+F, Ctrl+Shift+M, Ctrl+Shift+T, Ctrl+`, F5, F9, Escape) interceptados no nível do documento para evitar comportamentos padrão do navegador
...
- **Colaboração em Tempo Real** — Vários usuários podem editar o mesmo arquivo simultaneamente com rastreamento de cursor ao vivo, destaque de seleção e indicadores de presença. Alimentado por SignalR com transformação operacional para edições concorrentes sem conflitos. Cursores codificados por cor com rótulos de nome de usuário e reconexão automática
...
- **Multi-idioma / i18n** — Localização completa em todas as mais de 30 páginas, incluindo o IDE Web, com 1.088 chaves de recurso. Inclui 11 idiomas: inglês, espanhol, francês, alemão, japonês, coreano, chinês (simplificado), português, russo, italiano e turco. Seletor de idioma no cabeçalho e na barra superior do IDE. Adicione mais criando arquivos `SharedResource.{locale}.resx`
...
- **Sitemap.xml** — Sitemap XML dinâmico em `/sitemap.xml` listando todos os repositórios públicos para indexação por mecanismos de busca
